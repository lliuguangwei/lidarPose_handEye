...
static bool flag_time_1 = true;
static bool flag_time_2 = true;
static bool flag_time_3 = true;

std::string lidar_pose_hdl = "/home/lgw/Documents/project/ndt_mapping/pcd/lidar_pose_hdl.txt";
std::ofstream hdl_pose_outFile(lidar_pose_hdl.c_str(), std::ios::out);
...
public:
  ...
  virtual void onInit() {
    ...
    // subscribers
    odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(nh, "/odom", 10032));
    cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, "/filtered_points", 10032));
    sync.reset(new message_filters::TimeSynchronizer<nav_msgs::Odometry, sensor_msgs::PointCloud2>(*odom_sub, *cloud_sub, 10032));
    sync->registerCallback(boost::bind(&HdlGraphSlamNodelet::cloud_callback, this, _1, _2));

    // message_filters::Subscriber<nav_msgs::Odometry> odom_sub_2(nh, "odom", 1);
    // message_filters::Subscriber<sensor_msgs::PointCloud2> cloud_sub_2(nh, "filtered_points", 1);
    // TimeSynchronizer<nav_msgs::Odometry, sensor_msgs::PointCloud2> sync(odom_sub_2, cloud_sub_2, 10);
    // sync.registerCallback(boost::bind(&HdlGraphSlamNodelet::cloud_callback, _1, _2));

    floor_sub = nh.subscribe("/floor_detection/floor_coeffs", 10032, &HdlGraphSlamNodelet::floor_coeffs_callback, this);
...
private:
  ...
  // void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
  ...
  void map_points_publish_timer_callback(const ros::WallTimerEvent& event) {
    ...
    // auto cloud = map_cloud_generator->generate(snapshot, 1);
    if(!cloud) {
      return;
    }
    ...
    auto time_finish = ros::Time::now();
    // auto tmp_time = 1534087799;
    // std::cout << "time: " << time_finish.toSec() << std::endl;
    if(time_finish.toSec() > 1534360856 && flag_time_1 == true){
      std::cout << "savePCDFileBinary" << std::endl;
      // string PCDName = "/home/lgw/Documents/project/catkin_calibration/000.pcd";
      pcl::io::savePCDFileBinary("/home/lgw/Documents/project/catkin_calibration/000.pcd", *cloud);
      std::cout << "savePCDFileBinary  OK" << std::endl;
      flag_time_1 = false;
    }
    if((time_finish.toSec() > 1534346456) && (flag_time_2 == true)){
      std::cout << "savePCDFileBinary" << std::endl;
      // string PCDName = "/home/lgw/Documents/project/catkin_calibration/111.pcd";
      pcl::io::savePCDFileBinary("/home/lgw/Documents/project/catkin_calibration/111.pcd", *cloud);
      std::cout << "savePCDFileBinary  OK" << std::endl;
      // ros::shutdown();
      flag_time_2 = false;
    }
    if((time_finish.toSec() > 1534371656) && (flag_time_3 == true)){
      std::cout << "savePCDFileBinary" << std::endl;
      // string PCDName = "/home/lgw/Documents/project/catkin_calibration/111.pcd";
      pcl::io::savePCDFileBinary("/home/lgw/Documents/project/catkin_calibration/222.pcd", *cloud);
      std::cout << "savePCDFileBinary  OK" << std::endl;
      // ros::shutdown();
      flag_time_3 = false;
    }
  ...
  void optimization_timer_callback(const ros::TimerEvent& event) {
    ...
    Eigen::Isometry3d matric_1 = keyframe->node->estimate();

    hdl_pose_outFile << std::setprecision(20) << keyframe->stamp.toSec() << " ";
    hdl_pose_outFile << std::setprecision(12) << matric_1(0, 3) << " ";
    hdl_pose_outFile << std::setprecision(12) << matric_1(1, 3) << " ";
    hdl_pose_outFile << std::setprecision(12) << matric_1(2, 3) << std::endl;
    // hdl_pose_outFile << std::setprecision(12) << current_pose.roll << " ";
    // hdl_pose_outFile << std::setprecision(12) << current_pose.pitch << " ";
    // hdl_pose_outFile << std::setprecision(12) << current_pose.yaw << std::endl;
